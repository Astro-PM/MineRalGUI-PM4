...
class Main extends PluginBase implements Listener{
	...
	public function onbreak(BlockBreakEvent $ev){
	    ...
	    if($this->auto->get($player->getName()) == "on"){
	        if($ev->getBlock()->getId() == 56){
	            $ev->setDrops(array());
	            $this->data->set(self::DIAMOND, ($this->data->get(self::DIAMOND) + 1));
                $this->data->save();
            }
      //Than
            if($ev->getBlock()->getId() == 16){
                $ev->setDrops(array());     
                $this->data->set(self::COAL, ($this->data->get(self::COAL) + 1));
                $this->data->save();
            }
      ///Block Vàng
      ...
      //Đá Đỏ
            if($ev->getBlock()->getId() == 73){
                $ev->setDrops(array());
	            $this->data->set(self::REDSTONE, ($this->data->get(self::REDSTONE) + 1 * 4));
                $this->data->save();
            }
      //block emrald
            ...
            if($ev->getBlock()->getId() == 129){
                $ev->setDrops(array());
	            $this->data->set(self::EMERALD, ($this->data->get(self::EMERALD) + 1));
                $this->data->save();
            }
    ...
    public function addNumber64(int $type, Player $player) {
         $this->data = new Config($this->getDataFolder(). "players/" . $player->getName().".yml",Config::YAML);              $this->data->set($type, ($this->data->get($type) + 64 ));
         $this->data->save();
    }
    
    public function addNumber48(int $type, Player $player) {
         $this->data = new Config($this->getDataFolder(). "players/" . $player->getName().".yml",Config::YAML);              $this->data->set($type, ($this->data->get($type) + 48 ));
         $this->data->save();
    }
    
    public function addNumber32(int $type, Player $player) {
         $this->data = new Config($this->getDataFolder(). "players/" . $player->getName().".yml",Config::YAML);              $this->data->set($type, ($this->data->get($type) + 32 ));
         $this->data->save();
    }
    
    public function addNumber16(int $type, Player $player) {
         $this->data = new Config($this->getDataFolder(). "players/" . $player->getName().".yml",Config::YAML);              $this->data->set($type, ($this->data->get($type) + 16 ));
         $this->data->save();
    }
    
    public function addNumber1(int $type, Player $player) {
         $this->data = new Config($this->getDataFolder(). "players/" . $player->getName().".yml",Config::YAML);              $this->data->set($type, ($this->data->get($type) + 1 ));
         $this->data->save();
    }
    ...
    public function item(Player $sender)
    {
        ...
        $inventory->setItem($i, ItemFactory::getInstance()->get(160, 9, 1)->setCustomName(" §r §7 §r"));
         ...
         $inventory->setItem(10, ItemFactory::getInstance()->get(0, 8, 1) );
         $inventory->setItem(11, ItemFactory::getInstance()->get(0, 8, 1) );$inventory->setItem(12, ItemFactory::getInstance()->get(0, 9, 1) );
